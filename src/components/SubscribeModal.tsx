...
const plans = [
  ...
  },
  {
    ...
    price: 999,
    period: '/mo',
    features: ['Organization Dashboard', 'Member Data Export', 'Featured Listing', 'Dedicated Manager'],
    color: '#F59E0B'
  }
];
  ...
  const handleSubscribe = async (planId: string, price: number) => {
    ...
    try {
      await donationService.createOrder({
        amount: price * 100, // INR to paise
        isSubscription: true,
        planId,
        onSuccess: () => {
          setLoading(null);
          onClose(); // In a real app, maybe fetch updated user profile
        },
        onError: (err: any) => {
          setLoading(null);
              ...
              >
                ...
                <Box sx={{ flexGrow: 1, mb: 3 }}>
                  {plan.features.map((feature, idx) => (
                    <Box key={idx} sx={{ display: 'flex', alignItems: 'center', mb: 1.5 }}>
                      <CheckCircle sx={{ color: plan.color, fontSize: 20, mr: 1.5 }} />
                ...
                <Button 
                  ...
                  sx={{ 
                    ...
                    '&:hover': {
                      bgcolor: plan.popular ? '#312E81' : 'rgba(0,0,0,0.04)',
                      borderColor: plan.color,
                    }
                  }}
                >
